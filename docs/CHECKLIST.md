...
- [x] **Endurecimento de erro** (prática de mercado) — (a) **deadline gRPC** default 2s no Gateway (`DeadlineClientInterceptor` global) → estouro vira 504, protege o thread pool sob carga; (b) os 3 serviços gRPC **não vazam** exceção crua: `log.error` com stack + `INTERNAL` genérico (authz ganhou try/catch); (c) logs JSON (`logback-spring.xml` + logstash) nos 4 serviços → `log.error` correlacionável por `trace_id` no Loki. `DeadlineClientInterceptorTest` ✅. _Arthur adiantou._
...
- **Frontend real + no K8S** ✅ (2026-07-12) — SPA React/Vite integrada de verdade: login OIDC no Keycloak, 3 jornadas contra o gateway, adapter FHIR→UI (`lib/cohort.ts`), listas de pacientes/projetos. **Backend:** CORS no gateway + `Patient.meta.security` (bônus FHIR). Containerizado (`k8s/base/frontend.yaml`, `make deploy`). Login OIDC no browser via **split-horizon** (browser usa `localhost:8080`, gateway valida esse issuer mas busca JWKS no Service interno `keycloak:8080` — padrão de prod, sem editar hosts). Bônus: coorte com setor+medicamentos (exemplo §2.1 ii). Ver `docs/RUNBOOK-frontend.md`.
- ~~**CORS quebrou o boot do gateway** (CrashLoopBackOff)~~ ✅ **resolvido** (2026-07-12) — o `.cors(...)` injetava `CorsConfigurationSource` **por tipo**, mas há **2 beans** desse tipo no contexto (o `corsConfigurationSource` nosso + o `mvcHandlerMappingIntrospector` do Spring MVC) → `UnsatisfiedDependencyException` no startup. Fix: `.cors(Customizer.withDefaults())`, que resolve **por nome**. ⚠️ **Não pegou nos testes** porque o gateway não tem `@SpringBootTest` que sobe o web context inteiro — falha só em runtime. _Lição p/ o relatório: teste de fumaça de contexto (`@SpringBootTest`) pegaria isso._
- **`kind load` × attestation manifest (Docker buildkit)** 🟡 — o `docker build` moderno exporta um **manifest list com attestations** que o containerd do kind pode não conseguir rodar (só as imagens **construídas** falham; upstream como Postgres/Keycloak sobem). Mitigado: `make images` builda com **`--provenance=false`** (imagem single-manifest). Sintoma se reaparecer: pod `ErrImageNeverPull`/`CreateContainerError`.
